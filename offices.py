...
AIRTABLE_TABLE_NAME = "tblDUt6UGHvLJPQzr"
...
for broker in broker_elements:
    ...
    try:
        payment_listrows = broker.find("div", title="Offices in").find_all('div', class_="tooltip")
    except:
        continue
    payments = []
    payments_broker=[]
    for row in payment_listrows:
        paymentType={}
        payment_type_name = row.find("div", class_="tooltip__modal").get_text(strip=True)
        image = row.find('img')['src']
        paymentType['Name'] = payment_type_name
        paymentType['Icon'] = "https://www.earnforex.com" + image
