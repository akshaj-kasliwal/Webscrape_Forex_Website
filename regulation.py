...
AIRTABLE_TABLE_NAME = "tbl8i1RFkqbwMcber"
...
for broker in broker_elements:
    ...
    try:
        ...
        for d in div:
            regu = {}
            regu['Name'] = d.get_text(strip=True)
            if not "Regulation" in regu['Name']:
                regulations.append(regu)
    except:
        continue
# Remove dupes
res_list = [i for n, i in enumerate(regulations)
            if i not in regulations[n + 1:]]
